...
export class EDTAA3 {
    ...
    public RenderSDF(texture: RenderTexture, maxDist: number = 8): { texture: RenderTexture, alpha: Uint8ClampedArray } {
        ...
        if (this._useDualChannel) {
            // 手动插值需要用point采样
            resultTexture.setFilters(Texture2D.Filter.NEAREST, Texture2D.Filter.NEAREST);
        }

        resultTexture.initWithData(alpha, Texture2D.PixelFormat.RGBA8888, width, height);
        resultTexture.packable = false;
        // resultTexture.initWithData(imgData, Texture2D.PixelFormat.I8, width, height);
        return {
